...
class MNIST_MLP(object):
    ...
    def train(self):
        ...
        for idx_epoch in range(self.max_epoch):
            if idx_epoch in (8, 12):
                self.lr *= 0.5
            self.shuffle_data()
            for idx_batch in range(max_batch):
                    #print(f'[LR Decay] epoch {idx_epoch}, lr -> {self.lr:.5f}')
    ...
    def evaluate(self):
        ...
        nb = self.batch_size
        # 完整批
        for idx in range(n // nb):
            batch_images = self.test_data[idx*nb:(idx+1)*nb, :-1]
            prob = self.forward(batch_images)
            pred_labels = np.argmax(prob, axis=1)
            pred_results[idx*nb:(idx+1)*nb] = pred_labels
        # 余数批
        rem = n % nb
        if rem:
            batch_images = self.test_data[-rem:, :-1]
            prob = self.forward(batch_images)
            pred_labels = np.argmax(prob, axis=1)
            pred_results[-rem:] = pred_labels
...
def build_mnist_mlp(param_dir='weight.npy'):
    h1, h2, e = 512, 256, 15 
    mlp = MNIST_MLP(hidden1=h1, hidden2=h2, max_epoch=e, lr=0.1, print_iter=100)
    mlp.load_data()
    mlp.build_model()
    mlp.init_model()
    mlp.train()
